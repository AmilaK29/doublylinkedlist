...
namespace DoublyLinkedList
{
    public class DoublyLinkedList<T>
    {
        ...
        {
            if (node == null) throw new ArgumentNullException(); // Null check
            Node<T> current = node as Node<T>; // Type conversion
            if (current == null || current.Previous == null || current.Next == null) // Validate node links
                throw new InvalidOperationException();
            return current.Previous == Head ? null : current.Previous; // Return previous node (null if head)
        }

        public INode<T> AddFirst(T value)
        {
            return AddBetween(value, Head, Head.Next); // Insert between head and first node
        }

        public INode<T> AddBefore(INode<T> before, T value)
        {
            if (before == null) throw new ArgumentNullException(); // Null check
            Node<T> current = before as Node<T>; // Type conversion
            if (current == null || current.Previous == null || current.Next == null) // Validate links
                throw new InvalidOperationException();
            return AddBetween(value, current.Previous, current); // Insert before target node
        }

        public INode<T> AddAfter(INode<T> after, T value)
        {
            if (after == null) throw new ArgumentNullException(); // Null check
            Node<T> current = after as Node<T>; // Type conversion
            if (current == null || current.Previous == null || current.Next == null) // Validate links
                throw new InvalidOperationException();
            return AddBetween(value, current, current.Next); // Insert after target node
        }

        public void Clear()
        {
            Node<T> current = Head.Next; // Start at first node
            while (current != Tail) // Until tail sentinel
            {
                Node<T> next = current.Next; // Save next reference
                current.Next = null; // Clear forward link
                current.Previous = null; // Clear backward link
                current = next; // Move to next node
            }
            Head.Next = Tail; // Reset head's next
            Tail.Previous = Head; // Reset tail's previous
        ...
        {
            if (node == null) throw new ArgumentNullException(); // Null check
            if (node is not Node<T> current || current.Previous == null || current.Next == null) // Validate links
                throw new InvalidOperationException();

            current.Previous.Next = current.Next; // Bypass node backward
            current.Next.Previous = current.Previous; // Bypass node forward
            current.Next = null; // Clear forward link
            current.Previous = null; // Clear backward link
        ...
        {
            Node<T> node = Find(value) as Node<T>; // Find node by value
            if (node == null) throw new InvalidOperationException("The value is not in the list");
            Remove(node); // Delegate to node removal
        }

        public void RemoveFirst()
        {
            if (Count == 0) throw new InvalidOperationException(); // Empty list check
            Remove(Head.Next); // Remove node after head
        }

        public void RemoveLast()
        {
            if (Count == 0) throw new InvalidOperationException(); // Empty list check
            Remove(Tail.Previous); // Remove node before tail
        }
